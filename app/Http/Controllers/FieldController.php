...
class FieldController extends Controller
{
    ...
    public function requiredData(Request $request){
        if($request->ajax()){
            $result = FormField::find($request->id);
            if($result){
                $result->update(['required'=>$request->required]);
                return $this->response_json('success','Request field updated successful.',null,204);
            }else{
                return $this->response_json('error','Data Cannot Update!',null,204);
            }
        }
    }
}
