<div class="form-group {{ $colClass ?? '' }}">
    ...
    <textarea name="{{ $name }}" @if(!empty($rows)) rows="{{ $rows }}" @endif id="{{ $name }}" class="form-control form-control-sm rounded-0 shadow-none {{ $class ?? '' }}" @if(!empty($placeholder)) placeholder="{{ $placeholder }}" @endif>@if(!empty($value)) {{ $value }} @endif</textarea>
