                    ...
                    <div class="card mb-3" data-id="{{ $field->id }}">
                        <div class="card-body">
                            ...
                            <div class="custom-control custom-switch float-right">
                                <input type="checkbox" data-id="{{ $field->id }}" class="custom-control-input required-btn" {{ $field->required == 1 ? 'checked' : '' }} id="is-required{{ $field->id }}">
                                <label class="custom-control-label" for="is-required{{ $field->id }}">Is Required?</label>
                            </div>
                        </div>
                    </div>
                @endforeach
...
@push('scripts')
    ...
    <script>
        ...
        $(document).on('click','.required-btn',function(){
            ...
            $.ajax({
                url: "{{ route('app.forms.fields.required') }}",
                type: "POST",
                data: {id:id,required:required,_token:_token},
                dataType: "JSON",
            }).done(function (response) {
                if (response.status == "success") {
                    Swal.fire("Required", response.message, "success").then(function () {
                        window.location.reload();
                    });
                }
