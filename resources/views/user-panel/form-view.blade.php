                    ...
                    <form method="POST" id="form-submit" enctype="multipart/form-data">
                        ...
                        @foreach ($form->fields as $key=>$field)
                            <div class="form-group">
                                ...
                                <input type="hidden" name="field_type_{{ $field->id }}" value="{{ $field->type }}">
                                @if (in_array($field->type,['text','textarea','file','date','tel','email']))
                                    @if ($field->type == 'textarea')
                                        <textarea name="field_{{ $field->id }}" rows="3" class="form-control form-control-sm" @if($field->required == 1) required @endif @if($field->placeholder) placeholder="{{ $field->placeholder }}" @endif></textarea>
                                    @else
                                        <input type="{{ $field->type }}" name="field_{{ $field->id }}" class="form-control form-control-sm" @if($field->required == 1) required @endif @if($field->placeholder) placeholder="{{ $field->placeholder }}" @endif>
                                    @endif
                                @endif

                                @if (in_array($field->type,['select','radio','checkbox']))
                                    @if ($field->type == 'select')
                                        <select name="field_{{ $field->id }}[]" class="form-control form-control-sm" @if($field->required == 1) required @endif @if($field->multiple == 1) multiple="multiple" @endif>
                                    ...
                                    @else
                                        @foreach (explode(',',$field->options) as $key=>$value)
                                            <div class="form-check">
                                                ...
                                                <input class="form-check-input" name="field_{{ $field->id }}[]" type="{{ $field->type }}" value="{{ $value }}">
                                                @else
                                                <input class="form-check-input" name="field_{{ $field->id }}" type="{{ $field->type }}" value="{{ $value }}">
                                                @endif
